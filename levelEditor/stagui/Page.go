...
func (p *Page) Update(curMousePos [2]int) (string, *Button, *Slider, *InlineTextBox) {
	for _, s := range p.Sliders {
		if s.Update(curMousePos) {
			return s.Name, nil, s, nil
		}
	}
	...
	for _, b := range p.Buttons {
		if b.CheckClick(float32(curMousePos[0]), float32(curMousePos[1])) {
			return b.Name, b, nil, nil
		}
	}

	// Mouse press
	for _, itb := range p.InlineTextBoxes {
		if itb.CheckClick(curMousePos[0], curMousePos[1]) {
			return itb.Name, nil, nil, itb
		}
	}
